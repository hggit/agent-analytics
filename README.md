...
* **`apps/api`:** The Express API server backed by a durable local DuckDB store (`data/db.duckdb`).
* **`apps/web`:** The Vite React web client (proxies API requests to localhost:3000).
...
### 2. Installation
Install all dependencies in the workspaces monorepo from the root directory:
```bash
npm install
```
*(DuckDB native node bindings will be resolved and downloaded, or built locally if needed)*.
...
### 4. Start Dev Servers
Start both the Express API server and the Vite React frontend dev server concurrently:
```bash
npm run dev
```
...
While the dev servers are running, you can seed the DuckDB database by running the simulator from a separate terminal window:
